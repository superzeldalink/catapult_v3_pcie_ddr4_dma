...
//request_irq(bk_ptr->irq_line, dma_isr, IRQF_SHARED, ALTERA_DMA_DRIVER_NAME, (void *)bk_ptr); 
    ...
    bk_ptr->table_rd_cpu_virt_addr = ((struct dma_desc_table *)dma_alloc_coherent(&dev->dev, sizeof(struct dma_desc_table), &bk_ptr->table_rd_bus_addr, GFP_KERNEL));
    bk_ptr->lite_table_rd_cpu_virt_addr = ((struct lite_dma_desc_table *)dma_alloc_coherent(&dev->dev, sizeof(struct lite_dma_desc_table), &bk_ptr->lite_table_rd_bus_addr, GFP_KERNEL));
    if (!bk_ptr->table_rd_cpu_virt_addr || !bk_ptr->lite_table_rd_cpu_virt_addr) {
        rc = -ENOMEM;
        goto err_rd_table;
    }
    bk_ptr->table_wr_cpu_virt_addr = ((struct dma_desc_table *)dma_alloc_coherent(&dev->dev, sizeof(struct dma_desc_table), &bk_ptr->table_wr_bus_addr, GFP_KERNEL));
    bk_ptr->lite_table_wr_cpu_virt_addr = ((struct lite_dma_desc_table *)dma_alloc_coherent(&dev->dev, sizeof(struct lite_dma_desc_table), &bk_ptr->lite_table_wr_bus_addr, GFP_KERNEL));
    ...
    bk_ptr->rp_rd_buffer_virt_addr = dma_alloc_coherent(&dev->dev, PAGE_SIZE*bk_ptr->numpages, &bk_ptr->rp_rd_buffer_bus_addr, GFP_KERNEL);
    if (!bk_ptr->rp_rd_buffer_virt_addr) {
        rc = -ENOMEM;
        goto err_rd_buffer;
    }
    bk_ptr->rp_wr_buffer_virt_addr = dma_alloc_coherent(&dev->dev, PAGE_SIZE*bk_ptr->numpages, &bk_ptr->rp_wr_buffer_bus_addr, GFP_KERNEL);
...
err_wr_buffer:
    ...
    dma_free_coherent(&dev->dev, PAGE_SIZE*bk_ptr->numpages, bk_ptr->rp_rd_buffer_virt_addr, bk_ptr->rp_rd_buffer_bus_addr);
err_rd_buffer:
    dev_err(&dev->dev, "goto err_rd_buffer");
    dma_free_coherent(&dev->dev, sizeof(struct dma_desc_table), bk_ptr->table_wr_cpu_virt_addr, bk_ptr->table_wr_bus_addr);
err_wr_table:
    dev_err(&dev->dev, "goto err_wr_table");
    dma_free_coherent(&dev->dev, sizeof(struct dma_desc_table), bk_ptr->table_rd_cpu_virt_addr, bk_ptr->table_rd_bus_addr);
err_rd_table:
...
static void __exit altera_pci_remove(struct pci_dev *dev)
{
    ...
    dma_free_coherent(&dev->dev, sizeof(struct dma_desc_table), bk_ptr->table_rd_cpu_virt_addr, bk_ptr->table_rd_bus_addr);
    dma_free_coherent(&dev->dev, sizeof(struct lite_dma_desc_table), bk_ptr->lite_table_rd_cpu_virt_addr, bk_ptr->lite_table_rd_bus_addr);
    dma_free_coherent(&dev->dev, sizeof(struct dma_desc_table), bk_ptr->table_wr_cpu_virt_addr, bk_ptr->table_wr_bus_addr);
    dma_free_coherent(&dev->dev, sizeof(struct lite_dma_desc_table), bk_ptr->lite_table_wr_cpu_virt_addr, bk_ptr->lite_table_wr_bus_addr);
    dma_free_coherent(&dev->dev, PAGE_SIZE*bk_ptr->numpages, bk_ptr->rp_rd_buffer_virt_addr, bk_ptr->rp_rd_buffer_bus_addr);
    dma_free_coherent(&dev->dev, PAGE_SIZE*bk_ptr->numpages, bk_ptr->rp_wr_buffer_virt_addr, bk_ptr->rp_wr_buffer_bus_addr);
...
static int diff_timeval(struct timespec64 *result, struct timespec64 *t2, struct timespec64 *t1)
{
    s64 diff = (t2->tv_nsec + NSEC_PER_SEC * t2->tv_sec) - (t1->tv_nsec + NSEC_PER_SEC * t1->tv_sec);
