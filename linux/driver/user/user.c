...
void format_time_display(long seconds, long nanoseconds, char *output, size_t output_size);
...
void print_menu (char *buf) {
        ...
        printf("** %d) set num dwords (256 - 2048)           **\n", ALTERA_CMD_MODIFY_NUM_DWORDS                 );
        printf("** %d) set num descriptors (1 - 128)         **\n", ALTERA_CMD_MODIFY_NUM_DESC                   );
...
// Each loop contains 2 runs to test wrap around of last descriptor pointer
        char read_time_str[100], write_time_str[100], simul_time_str[100];
        format_time_display(((struct dma_status *)buf)->read_time.tv_sec, ((struct dma_status *)buf)->read_time.tv_nsec, read_time_str, sizeof(read_time_str));
        format_time_display(((struct dma_status *)buf)->write_time.tv_sec, ((struct dma_status *)buf)->write_time.tv_nsec, write_time_str, sizeof(write_time_str));
        format_time_display(((struct dma_status *)buf)->simul_time.tv_sec, ((struct dma_status *)buf)->simul_time.tv_nsec, simul_time_str, sizeof(simul_time_str));
        printf("Read Time               : %s\n", read_time_str);
        printf("Read Throughput         : %f GB/S\n", (((struct dma_status *)buf)->length_transfer)/(1024.0*1024.0)/((double)((struct dma_status *)buf)->read_time.tv_nsec/1000000000.0 + ((struct dma_status *)buf)->read_time.tv_sec) );
        printf("Write Time              : %s\n", write_time_str);
        printf("Write Throughput        : %f GB/S\n", (((struct dma_status *)buf)->length_transfer)/(1024.0*1024.0)/((double)((struct dma_status *)buf)->write_time.tv_nsec/1000000000.0 + ((struct dma_status *)buf)->write_time.tv_sec) );
        printf("Simultaneous Time       : %s\n", simul_time_str);
        printf("Simultaneous Throughput : %f GB/S\n", (((struct dma_status *)buf)->length_transfer*2.0)/(1024.0*1024.0)/((double)((struct dma_status *)buf)->simul_time.tv_nsec/1000000000.0 + ((struct dma_status *)buf)->simul_time.tv_sec) );
    ...
    do {
        ...
        switch (num_input)
        {
          ...
          case ALTERA_CMD_MODIFY_NUM_DWORDS:
                ...
                num_input = strtol(line, NULL, 10);
                *(int *)buf = num_input;
		 if (num_input < 256 || num_input > 2048){
		    printf("the maximum transfer size of each descriptor is 2048 DW (8kB) for the 128-bits interface \n");
		    sleep (1);
                    break;}
                else
			write (f, &cmd, 0);
           ...
           case ALTERA_CMD_MODIFY_NUM_DESC:
		    ...
		    printf("the maximum number of descriptor is 128\n");
		    sleep (1);
                    break;}
                else
                    write (f, &cmd, 0);
...
void format_time_display(long seconds, long nanoseconds, char *output, size_t output_size) {
    if (seconds > 0) {
        snprintf(output, output_size, "%ld s and %ld ns", seconds, nanoseconds);
    } else if (nanoseconds >= 1000000000L) {  // 1 second in nanoseconds
        double total_seconds = seconds + nanoseconds / 1000000000.0;
        snprintf(output, output_size, "%g s", total_seconds);
    } else if (nanoseconds >= 1000000L) {  // 1 millisecond in nanoseconds
        long milliseconds = nanoseconds / 1000000;
        long remaining_ns = nanoseconds % 1000000;
        if (remaining_ns > 0) {
            snprintf(output, output_size, "%ld ms and %ld ns", milliseconds, remaining_ns);
        } else {
            snprintf(output, output_size, "%ld ms", milliseconds);
        }
    } else if (nanoseconds >= 1000L) {  // 1 microsecond in nanoseconds
        long microseconds = nanoseconds / 1000;
        long remaining_ns = nanoseconds % 1000;
        if (remaining_ns > 0) {
            snprintf(output, output_size, "%ld us and %ld ns", microseconds, remaining_ns);
        } else {
            snprintf(output, output_size, "%ld us", microseconds);
        }
    } else {
        snprintf(output, output_size, "%ld ns", nanoseconds);
    }
}
